    ...
    public enum Mail: Error, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .malformedEmailAddress(let email):
                return "\(email) is not a valid email address. Please provide an RFC 5322 compliant email address."
            ...
            case .thresholdOutOfRange(let threshold):
                return "The spam checker app only accepts a threshold which is between 1 and 10 (attempted to use \(threshold))"
            ...
            case .headerNotAllowed(let header):
                return String(format: "The \"\(header)\" header is a reserved header, and cannot be used in the `headers` property.")
                
            case .malformedHeader(let header):
                return String(format: "Invalid header \"\(header)\": When defining the headers that you would like to use, you must make sure that the header's name contains only ASCII characters and no spaces.")
            ...
            case .categoryTooLong(let category):
                return "A category cannot have more than \(Constants.Categories.CharacterLimit) characters (attempted to use category named \"\(category)\")."
            ...
            case .invalidContentType(let contentType):
                return "Invalid content type \"\(contentType)\": Content types cannot contain ‘;’, spaces, or CRLF characters, and must be at least 1 character long."
                
            case .duplicateRecipient(let recipient):
                return "Each unique email address in the `personalizations` array should only be included once. You have included \"\(recipient)\" more than once."
            ...
            case .invalidContentID(let contentID):
                return "Invalid content ID \"\(contentID))\" for attachment: Content IDs cannot contain ‘;’, spaces, or CRLF characters, and must be at least 1 character long."

            case .invalidFilename(let filename):
                return "Invalid filename \(filename) for attachment: Filenames cannot contain ‘;’, spaces, or CRLF characters, and must be at least 1 character long."
            }
        }
    }
}
